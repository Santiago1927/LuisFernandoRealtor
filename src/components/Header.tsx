      ...
      <header className="bg-white dark:bg-black shadow-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-16">
          ...
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="font-bold text-yellow-500 text-xl">LuisFernandoRealtor</span>
          </Link>
          {/* Navegación principal - visible solo en pantallas medianas y grandes */}
          <nav className="hidden md:flex gap-8 items-center">
            <Link href="/" className="hover:text-yellow-500 dark:hover:text-yellow-400 font-medium">Inicio</Link>
            <Link href="/propiedades" className="hover:text-yellow-500 dark:hover:text-yellow-400 font-medium">Propiedades</Link>
            <Link href="/contacto" className="hover:text-yellow-500 dark:hover:text-yellow-400 font-medium">Contacto</Link>
            {isAuthenticated && (
              <button
                onClick={handleAdminPanel}
                className="ml-4 px-4 py-2 bg-yellow-500 text-black rounded-lg hover:bg-yellow-400 transition-colors font-bold shadow dark:bg-yellow-400 dark:hover:bg-yellow-300 dark:text-black"
              >
                Panel Administrador
              </button>
            )}
          ...
          <nav className="flex flex-col gap-4">
            ...
            {isAuthenticated && (
              <button
                ...
                className="mt-2 px-4 py-2 bg-yellow-500 text-black rounded-lg hover:bg-yellow-400 transition-colors font-bold shadow dark:bg-yellow-400 dark:hover:bg-yellow-300 dark:text-black"
              >
                Panel Administrador
              </button>
            )}
