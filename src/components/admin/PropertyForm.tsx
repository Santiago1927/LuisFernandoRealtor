...
const BUSINESS_TYPES: BusinessType[] = ["Venta", "Permuta"];
...
export default function PropertyForm({
  ...
  // Estado para controlar el diálogo de confirmación de cierre
  const [showCloseDialog, setShowCloseDialog] = useState(false);

  // Función para manejar el intento de cierre
  const handleCloseAttempt = () => {
    console.log("🚪 Intentando cerrar - hasUnsavedChanges:", hasUnsavedChanges);
    if (hasUnsavedChanges) {
      console.log("⚠️ Mostrando diálogo de confirmación");
      setShowCloseDialog(true);
    } else {
      console.log("✅ Cerrando sin cambios");
      handleClose();
    }
  };
  ...
  return (
    <>
      ...
      {/* Diálogo de confirmación para cerrar con cambios sin guardar */}
      <AlertDialog open={showCloseDialog} onOpenChange={setShowCloseDialog}>
        <AlertDialogContent className="bg-white dark:bg-zinc-900">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center text-zinc-900 dark:text-zinc-100">
              <AlertTriangle className="w-5 h-5 mr-2 text-yellow-600" />
              ¿Cerrar sin guardar?
            </AlertDialogTitle>
            <AlertDialogDescription className="text-zinc-700 dark:text-zinc-300">
              Tienes cambios sin guardar. Si cierras ahora, perderás todos los
              cambios realizados.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="border-zinc-300 dark:border-zinc-600">
              Continuar editando
            </AlertDialogCancel>
            <AlertDialogAction
