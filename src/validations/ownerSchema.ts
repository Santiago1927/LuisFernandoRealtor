...
const positiveNumberField = (message: string) =>
    z.string({ message })
        ...
        .refine(val => !val.includes('.'), { message })
        .refine(val => !val.includes(','), { message });

export const ownerSchema = z.object({
    firstQuestion: z.string({ message: 'Este campo es obligatorio' }).refine(val => val === 'true', { message: 'Lo siento, si no quieres vender tu propiedad rápidamente no te puedo ayudar' }),
    secondQuestion: z.string({ message: 'Este campo es obligatorio' }).refine(val => val === 'true', { message: 'Lo siento, si no quieres vender tu propiedad a un precio acorde al mercado no te puedo ayudar' }),
    nombre: z.string().max(200, { message: 'El nombre debe tener menos de 200 caracteres' }).min(1, { message: 'Este campo es obligatorio' }).refine(val => /^[a-zA-Z\s]*$/.test(val), { message: 'El nombre solo puede contener letras' }),
    correo: z.string().min(1, { message: 'Este campo es obligatorio' }).email({ message: 'El email debe ser válido' }),
    telefono: z.string().min(1, { message: 'Este campo es obligatorio' }).refine(val => /^[0-9]*$/.test(val), { message: 'El teléfono solo puede contener números' }),
    ...
    direccion: z.string().min(1, { message: 'Este campo es obligatorio' }),
